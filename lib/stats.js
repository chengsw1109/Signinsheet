...
function tzOffsetMs(d = new Date()) {
  ...
  // 時區偏移必為整分鐘；四捨五入到分鐘以消除毫秒誤差（否則計算出的時刻會帶不整齊的毫秒）
  const raw = Date.parse(s.replace(' ', 'T') + 'Z') - d.getTime();
...
// 依姓名／工號關鍵字篩選在職人員；q 留空回傳全部
function activePersons(q) {
  if (q) {
    return db.prepare(
      `SELECT * FROM persons WHERE active = 1 AND (name LIKE ? OR emp_no LIKE ?) ORDER BY emp_no`
    ).all(`%${q}%`, `%${q}%`);
  }
  return db.prepare(`SELECT * FROM persons WHERE active = 1 ORDER BY emp_no`).all();
}

// ---- 日統計：每人當日簽到/簽退時間、遲到、工時、狀態 ----
function dailyDetail(date, q) {
  const { startIso, endIso } = dayRangeUtc(date);
  const persons = activePersons(q);
  const rows = [];
...
function monthlyReport(month /* 'YYYY-MM' */, q) {
