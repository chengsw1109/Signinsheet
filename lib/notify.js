...
const HELP =
  `您好！可用指令：\n` +
  `・預約 → 取得線上預約連結\n` +
  `・綁定 工號 PIN碼 → 員工綁定帳號（例：綁定 A001 123456）\n` +
  `・查詢 → 查看今日簽到退記錄\n` +
  `・解除綁定 → 停止接收通知`;
...
async function handleLineEvent(ev) {
  ...
  // 客戶預約：回覆帶有本人 LINE ID 的預約連結，預約成功後即可收到 LINE 通知
  if (/^(預約|立即預約|我要預約)$/.test(text)) {
    if (!PUBLIC_URL) return reply('預約系統尚未設定對外網址，請聯絡管理員。');
    return reply(`請點選以下連結進行預約：\n${PUBLIC_URL}/booking.html?u=${encodeURIComponent(uid)}\n\n預約成功後會在此收到通知 😊`);
  }

  let m;
  if ((m = text.match(/^綁定\s+(\S+)\s+(\S+)$/))) {
    const person = db.prepare(`SELECT * FROM persons WHERE emp_no = ?`).get(m[1]);
    if (person && !person.active) return reply('此帳號已被停用，請聯絡管理員在後台重新「啟用」後再綁定。');
    if (!person || !verifyPin(m[2], person.pin_hash)) return reply('工號或 PIN 碼錯誤，綁定失敗。\n（若管理員重設過 PIN，請改用新的 PIN 碼）');
