...
- **📅 線上預約**（`/booking.html`，供客戶使用）
  - 客戶選擇服務 → 日期 → 可預約時段 → 填聯絡資料 → 送出
  - 系統自動排除已被預約的時段，避免撞單
  - 預約成立後同時：存入資料庫、（選用）加入 Google 日曆、（選用）寄 Email 給客戶、LINE 通知客戶與管理員
  - 客戶在 LINE 傳「預約」即取得帶有本人身分的預約連結，預約後可在 LINE 收到通知
  - 管理後台可設定服務項目、查詢與取消預約
...
1. 管理後台「服務項目」新增您提供的服務（名稱、時長、費用）。
2. 客戶預約管道：
   - 直接開 `/booking.html`（可放進 LINE 圖文選單、官網連結）
   - 或客戶在 LINE 傳「預約」，系統回覆帶有本人 LINE ID 的預約連結（需設定 `PUBLIC_URL`），這樣預約後客戶就能在 LINE 收到通知
3. 營業時間、可預約星期、時段間隔皆可用環境變數調整（見下表）。
4. Email 與 Google 日曆為選用，未設定時預約照常運作，只是不寄信／不寫入日曆：
   - **Email**：填 `SMTP_*` 環境變數（Gmail 需用「應用程式密碼」）
   - **Google 日曆**：建立 Google 服務帳戶、把日曆共用給服務帳戶信箱，填 `GOOGLE_SERVICE_ACCOUNT_JSON` 與 `GOOGLE_CALENDAR_ID`

| 變數 | 預設 | 說明 |
|---|---|---|
| `PUBLIC_URL` | （空） | 對外網址，LINE「預約」指令回覆連結用 |
| `BOOK_OPEN` / `BOOK_CLOSE` | `10:00` / `20:00` | 每日可預約起訖時間 |
| `BOOK_SLOT_MIN` | `30` | 預約時段間隔（分鐘） |
| `BOOK_WORKDAYS` | `1,2,3,4,5,6` | 可預約的星期（1=週一 … 7=週日） |
...
- **手動備份／下載**：管理後台「資料庫備份與還原」區塊可「立即備份」、下載任一份備份；建議定期下載到另一台電腦保存（異地備份）
- **線上還原**：備份清單中按「還原」即可將人員與簽到記錄回復到該備份的狀態，**不需停機**；還原前系統會自動把現況存成 `*-pre-restore.db` 安全備份，按錯可再從它還原回來
- **上傳還原**：之前下載到別台電腦的備份檔，可從介面上傳（會先驗證檔案完整性）後再還原
- 還原只影響人員與簽到記錄；登入狀態不受影響。備份使用 SQLite 線上備份 API，過程中系統照常運作
